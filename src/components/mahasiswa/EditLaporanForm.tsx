// src/components/mahasiswa/EditLaporanForm.tsx
"use client";
...
export default function EditLaporanForm({ initialData }: { initialData: any }) {
  const router = useRouter();

  const [semester, setSemester] = useState(initialData.semester || "");
  const [tahunAjaran, setTahunAjaran] = useState(initialData.tahunAjaran || "");
  ...
  function addItem() {
    setItems((prev) => [
      ...prev,
      { deskripsi: "", nominal: "", kategori: "LAINNYA" },
    ]);
  }
  ...
  function updateItem(index: number, field: keyof ItemLaporan, value: string) {
    setItems((prev) =>
      prev.map((item, idx) =>
  ...
  const totalDana = items.reduce(
    (sum, item) => sum + (parseFloat(item.nominal as string) || 0),
    0,
  );
  ...
  async function handleSubmit(submitStatus: "DRAF" | "TERKIRIM") {
    ...
    const invalidItems = items.filter(
      (i) =>
        !i.deskripsi.trim() ||
        !i.nominal ||
        parseFloat(i.nominal as string) <= 0,
    );
    ...
    const payload = {
      ...
      items: items.map((i) => ({
        ...
        nominal: parseFloat(i.nominal as string),
        kategori: i.kategori,
      })),
    };
    ...
    if (res.ok) {
      router.push(`/dashboard/mahasiswa/laporan/${initialData.id}`);
      router.refresh();
    } else {
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div className="flex items-center gap-3">
        <Link
          href={`/dashboard/mahasiswa/laporan/${initialData.id}`}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateItem(idx, "deskripsi", e.target.value)
                    }
                ...
                <div>
                  ...
                  <select
                    value={item.kategori}
                    onChange={(e) =>
                      updateItem(idx, "kategori", e.target.value)
                    }
                    className="input text-sm"
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={item.nominal}
                    onChange={(e) => updateItem(idx, "nominal", e.target.value)}
